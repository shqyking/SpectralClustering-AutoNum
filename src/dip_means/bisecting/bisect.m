...
% Algorithm:
%------------
% The algorithm is describes by the folowing steps:
%     1.    Supposing we have a set M of data points 
%           (splitMODE=0): Randomply select a data point pl in space -> (left centroid candidate)
%           Compute the centroid c of data points in M
%           Compute the point pr as: pb = c-(pl-c) -> (right centroid candidate)
%           (splitMODE=1): Randomply select two data points
%           (splitMODE=2): uses the sim/dist distribution of a split voter based on Hardigan Unimodality Test
...
% bestgIdx:        the asobject-to-cluster assignement in a vector
...
for i=1:splittrials,    % try to split the cluster a number of times...
    ...
    elseif (splitMODE == 1) % select two different data points
        d1 = randobjects(i);
        d2 = floor(1 + rand*n);
        while (d1 == d2), d2= floor(1 + rand*n); end   % guarrantee difference in seed objects
        c = full([ X(d1, :);     X(d2, :) ]);
    else%if (splitMODE == 2)
