...
% viewer point in cluster to all other members in that cluter.
%------------
% Input
%   D     : the dataset (row vectors)
%   nboot : the number of bootstrap uniform distributions to use
%   exhaustive_search: whether all viewers should be ckecked or to use some
...
%------------
function [maxdip, pmin, distr, viewers_for_split] = test_unimodal_cluster (D, nboot, exhaustive_search, voting)
n = size(D,1);
...
if (exhaustive_search == 0)
    ...
    % Setup the order in witch the objects (view points) should be checked for
