...
%                - exhaustive_search (default=1)  whether Hardigan test must be done for all cluster objects or to stop when first prob=0 is found
...
%                - nboot  (default=1000)          number of normal distribution samples to test each object with Hardigan test
%                - overall_distr (default=0)      if 1, Hardigan test is applied one time on the overall distribution of distances
...
%              (3) dip-means: the cluster with lower prob of unimodal sim/dist distribution (Hardigan)
...
%              (1) two randomply selected objects are the initial prototypes of the split.
%              (2) the split is based on Hardigan unimodality test
...
function [R, min_err, R_ref, min_err_ref] = bisect_kmeans (X, varargin)
...
if (splitSELECT == 3) % Hardigan unimodal test on similaritites/distances
    if (~isempty(split_struct))
        pval_threshold    = split_struct.pval_threshold;    % the probability <= to which the cluster must split
        exhaustive_search = split_struct.exhaustive_search; % whether Hardigan test must be done for all cluster objects or to stop when first prob=0 is found
        ...
        nboot             = split_struct.nboot;             % number of normal distribution samples to test each object with Hardigan test
        overall_distr     = split_struct.overall_distr;     % if 1, Hardigan test is applied one time on the overall distribution of distances
        clear('split_struct');
    else                        % default values
        pval_threshold    = 0;    
        exhaustive_search = 1;    
    ...
    if (kernelSpace == 0)% || (kernelSpace == 1 && splitMODE == 2)) % no kernel space is to be used
...
if (~kernelSpace && (splitSELECT == 3 || splitSELECT == 4)), c_all = ComputeCentroids(X, ones(n,1), 1, 0); end

min_err = bitmax;
min_err_ref = bitmax;
    
for exp_id=1:attempts,  % do a number of clustering attempts starting each time from the single cluster case
    %at fisrt all data points are assigned to one cluster
    gIdx = ones(n,1);
    ...
    Er = zeros(kmax,1);
    Er(1) = bitmax;       %the first cluster to be split
    k = 1;           %the number of clusters up to now
    if (~kernelSpace && (splitSELECT == 3 || splitSELECT == 4)),
                         c = c_all;
    else                 c = [];
    end
    ...
    while (k < kmax || splitSELECT > 2)
        ...
        case 3
            if (isempty(candidates)), 
                fprintf('-> Hardigan: Stopped splitting at k=%g  (smallest clusters reached)\n\n', k); 
                break;
            end;
            ...
            if (overall_distr == 0) % use individual object-viewers
                if (splitMODE == 2)
                    ...
                    for i=candidates,
                        [cluster_eval(i), pval(i)] = test_unimodal_cluster (D(clmemberIDs{i},clmemberIDs{i}), nboot, exhaustive_search, voting);
                    end
                end
            else
                for i=candidates,
                    ...
                    DD = DD(:);  DD = DD(DD~=0);
                    [cluster_eval(i), pval(i)] = HartigansDipSignifTest (DD, nboot);
                end
            end
            %[maxv, cltosplit] = max(cluster_eval); % select cluster with minimum p_value -> less probabble to be unimodal wrt similarities/distributions
            [~, cltosplit] = max(cluster_eval); % select cluster with minimum p_value -> less probabble to be unimodal wrt similarities/distributions
            
            minpv = pval(cltosplit);
            
            if (minpv > pval_threshold), 
                fprintf('-> Hardigan: Stopped splitting at k=%g\n\n', k); 
                break; 
            end
        case 4
            if (isempty(candidates)),
        ...
        if (isempty(bestclmemberIDs)) % bestclmemberIDs exists only for BIC (x-means) case
            ...
            else [bestclmemberIDs, ~, bestsplit_c, newEr] = bisect(X(clmemberIDs{cltosplit},:), splitMODE, splittrials, exps_group_id*exp_id*k);
            end
        end
